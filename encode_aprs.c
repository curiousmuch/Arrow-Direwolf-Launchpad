...
static int set_norm_position (char symtab, char symbol, double dlat, double dlong, int ambiguity, position_t *presult)
{
	...
	if (symtab != '/' && symtab != '\\' && ! isdigit(symtab) && ! isupper(symtab)) {
	  ...
	  printf ("Symbol table identifier is not one of / \\ 0-9 A-Z\n");
	}
	...
	if (symbol < '!' || symbol > '~') {
	  //text_color_set(DW_COLOR_ERROR);
	  printf ("Symbol code is not in range of ! to ~\n");
	}
...
/* Finally, comment text. */
	...
	if (result_len >= (int)result_size) {
	  //text_color_set(DW_COLOR_ERROR);
	  printf ("encode_position result of %d characters won't fit into space provided.\n", result_len);
	}
...
/* Finally, comment text. */
	...
	if (result_len >= (int)result_size) {
	  //text_color_set(DW_COLOR_ERROR);
	  printf ("encode_object result of %d characters won't fit into space provided.\n", result_len);
	}
...
/***********  Position  ***********/
	...
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
// TODO:  Need to test specifying some but not all.
	...
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&PHG7368") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/* with freq & tone.  minus offset, no offset, explict simplex. */
	...
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&146.955MHz T074 -060 ") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }

	encode_position (0, 0, 42+34.61/60, -(71+26.47/60), 0, G_UNKNOWN, 'D', '&',
		0, 0, 0, NULL, G_UNKNOWN, 0, 146.955, 74.4, G_UNKNOWN, NULL, result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&146.955MHz T074 ") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }

	encode_position (0, 0, 42+34.61/60, -(71+26.47/60), 0, G_UNKNOWN, 'D', '&',
		0, 0, 0, NULL, G_UNKNOWN, 0, 146.955, 74.4, 0, NULL, result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&146.955MHz T074 +000 ") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/* with course/speed, freq, and comment! */
	...
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&180/055146.955MHz T074 -060 River flooding") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/* Course speed, no tone, + offset */
	...
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&180/055146.955MHz +060 River flooding") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/* Course speed, no tone, + offset + altitude */
	...
	encode_position (0, 0, 42+34.61/60, -(71+26.47/60), 0, 12345, 'D', '&',
		0, 0, 0, NULL, 180, 55, 146.955, G_UNKNOWN, 0.6, "River flooding", result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&180/055146.955MHz +060 /A=012345River flooding") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }

	encode_position (0, 0, 42+34.61/60, -(71+26.47/60), 0, 12345, 'D', '&',
		0, 0, 0, NULL, 180, 55, 146.955, 0, 0.6, "River flooding", result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, "!4234.61ND07126.47W&180/055146.955MHz Toff +060 /A=012345River flooding") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/*********** Compressed position. ***********/
	...
	encode_position (0, 1, 42+34.61/60, -(71+26.47/60), 0, G_UNKNOWN, 'D', '&',
		0, 0, 0, NULL, G_UNKNOWN, 0, 0, 0, 0, NULL, result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, "!D8yKC<Hn[&  !") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/* with PHG. In this case it is converted to precomputed radio range.  TODO: check on this.  Is 27.4 correct? */
	...
	encode_position (0, 1, 42+34.61/60, -(71+26.47/60), 0, G_UNKNOWN, 'D', '&',
		50, 100, 6, "N", G_UNKNOWN, 0, 0, 0, 0, NULL, result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, "!D8yKC<Hn[&{CG") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/* with course/speed, freq, and comment!  Roundoff. 55 knots should be 63 MPH.  we get 62. */
	...
	encode_position (0, 1, 42+34.61/60, -(71+26.47/60), 0, G_UNKNOWN, 'D', '&',
		0, 0, 0, NULL, 180, 55, 146.955, 74.4, -0.6, "River flooding", result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, "!D8yKC<Hn[&NUG146.955MHz T074 -060 River flooding") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }
...
/*********** Object. ***********/
	...
	encode_object ("WB1GOF-C", 0, 0, 42+34.61/60, -(71+26.47/60), 0, 'D', '&',
		0, 0, 0, NULL, G_UNKNOWN, 0, 0, 0, 0, NULL, result, sizeof(result));
	printf ("%s\n", result);
	if (strcmp(result, ";WB1GOF-C *111111z4234.61ND07126.47W&") != 0) { printf ("ERROR!  line %d\n", __LINE__); errors++; }

// TODO: need more tests.

	if (errors > 0) {
	  //text_color_set (DW_COLOR_ERROR);
	  printf ("Encode APRS test FAILED with %d errors.\n", errors);
	  exit (EXIT_FAILURE);
	}

	//text_color_set (DW_COLOR_REC);
	printf ("Encode APRS test PASSED with no errors.\n");
	exit (EXIT_SUCCESS);
